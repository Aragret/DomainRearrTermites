        ...
        for raw_line in drf:
            if raw_line[0] not in ('#', '\n'):
                ...
                if line[0] in nodeIDs and line[1] in solutions_lst and line[2] in events_lst:
                    if line[2] == 'fission' or line[2] == 'terminal emergence' or line[2] == 'terminal loss':
                        node, solution, event, new_arrangement, old = line
                        drset.add(DRentry(node=node.strip(), solution=solution.strip(), event=event.strip(),
                                          arrangement='-'.join(old.strip().split())))
                    elif line[2] == 'fusion':
                        node, solution, event, new_arrangement, old = line
                        old_arrgmnts = old.strip().split('+')
                        drset.add(DRentry(node=node.strip(), solution=solution.strip(), event=event.strip(),
                                          arrangement='-'.join(old_arrgmnts[0].strip().split())))
                        drset.add(DRentry(node=node.strip(), solution=solution.strip(), event=event.strip(),
                                          arrangement='-'.join(old_arrgmnts[1].strip().split())))
                        ...
                        node, solution, event, new, old = line
                        drset.add(DRentry(node=node.strip(), solution=solution.strip(), event=event.strip(),
                                          arrangement='-'.join(old.strip().split())))
